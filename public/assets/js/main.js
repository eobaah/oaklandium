  ...
  var _ = {
    ...
    initDesktop: function() {
      ...
      _.objects.window._parallelism_update = function() {
        ...
        while (rows > _.settings.minRows && itemsWidth / rows < windowWidth) rows--;
        ...
        _.objects.items.each(function(i) {
          ...
          if (rowEnded) {
            ...
            _.objects.items.slice(rowStart, rowEnd + 1).each(function(j) {
              var $item = $(this);
              var p = $item.data('width') / w * 100.0;

              if (pt + p > 100.0 || (rowStart + j == rowEnd && pt + p < 100.0)) p = 100.0 - pt;
        ...
        // Resize reel.
        _.objects.reel.css('height', itemHeight * rows + _.settings.padding * 2).css('width', rowWidth);

        // Reposition main (if applicable).
        _.objects.main.css('height', itemHeight * rows + _.settings.padding * 2);
        ...
        window.setTimeout(function() {
          $SZ.css('height', _.objects.main.outerHeight()).css('top', _.objects.main.offset().top);
      ...
      _.objects.window.resize(function() {
        ...
        if (Math.max(Math.floor(windowHeight / itemHeight) - 1, 1) != rows) _.objects.window._parallelism_update();
      ...
      if (_.IEVersion < 9) _.objects.reel.show();
        ...
        window.setTimeout(function() {
          _.objects.reel
            .h5u_xcss('transition', 'opacity ' + _.settings.introSpeed / 1000.0 + 's ease-in-out')
            .css('opacity', 1);
        }, _.settings.introDelay);
      }
      ...
      _.objects.items.each(function(i) {
        ...
        if ($img.length > 0) {
          ...
          if (_.IEVersion < 9) $itemInner.show();
          else if (_.IEVersion < 10) {
            $itemInner.hide();
            ...
            $itemInner.h5u_xcss('transition', 'opacity ' + _.settings.thumbSpeed / 1000.0 + 's ease-in-out');
      ...
      else _.objects.main.css('overflow', 'hidden');

      // Scrolling.

      // Scroll Wheel.

      if (_.IEVersion < 9) _.objects.main.css('overflow-x', 'scroll');
      else {
        ...
        if (_.settings.scrollWheelTarget == 'reel') st = _.objects.main[0];
        else st = _.objects.window[0];
      ...
      if (!_.isTouch && _.settings.useScrollZones) {
        ...
        $SZ._parallelism_update = function() {
          if (_.objects.main.scrollLeft() == 0) $SZLeft.hide();
          else $SZLeft.show();

          if (_.objects.main.scrollLeft() + $(window).width() >= _.objects.reel.outerWidth()) $SZRight.hide();
          else $SZRight.show();
        };

        $SZRight.bind('mouseenter', function(e) {
          SZIntervalId = window.setInterval(function() {
            _.objects.main.scrollLeft(
              _.objects.main.scrollLeft() + _.settings.scrollZoneAmount * _.settings.scrollFactor
            );
            $SZ._parallelism_update();
      ...
      } else $SZ._parallelism_update = function() {};

      // Scroll Keys.
      if (_.settings.useScrollKeys) {
        _.objects.window.keydown(function(e) {
          if ($('.poptrox-popup').is(':visible')) return;

          switch (e.keyCode) {
            case 39:
              window.clearInterval(SZIntervalId);
              _.objects.main.scrollLeft(
                _.objects.main.scrollLeft() + _.settings.scrollKeyAmount * _.settings.scrollFactor
              );
              $SZ._parallelism_update();
              return false;
      ...
      _.objects.reel.poptrox({
        onPopupClose: _.settings.useBlurFilter
          ? function() {
              _.objects.wrapper.removeClass('overlayed');
            }
          : null,
        onPopupOpen: _.settings.useBlurFilter
          ? function() {
              _.objects.wrapper.addClass('overlayed');
            }
          : null,
    ...
    initMobile: function() {
      ...
      if (_.settings.autoStyleMobile) {
        // Items.
        _.objects.items.css('border', 'solid ' + Math.ceil(_.settings.padding / 2) + 'px ' + _.settings.paddingColor);
